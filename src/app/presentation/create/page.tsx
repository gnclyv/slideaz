  ...
  const handleDirectGeneration = async (promptText: string, lang: string) => {
    try {
      ...
      const result = await createEmptyPresentation({
        title: promptText.substring(0, 50) || "Adsız Təqdimat",
        theme: createTheme,
        language: lang,
      });
      ...
      if (result.success && result.presentation) {
        ...
        toast.error(result.message || "Təqdimat yaradıla bilmədi");
        router.push("/presentation");
      }
    } catch (error) {
      setIsGeneratingOutline(false);
      console.error("Error creating presentation:", error);
      toast.error("Təqdimat yaradıla bilmədi");
      router.push("/presentation");
    }
  };
      ...
      >
        <div className="flex h-[calc(100vh-8rem)] flex-col items-center justify-center">
          ...
          <div className="space-y-2 text-center">
            <h2 className="text-2xl font-bold">Təqdimat konturunun yüklənməsi</h2>
            <p className="text-muted-foreground">Bir az gözləyin...</p>
          </div>
        </div>
      </ThemeBackground>
    );
