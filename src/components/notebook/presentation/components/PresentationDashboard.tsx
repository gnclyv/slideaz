...
const LANGUAGE_OPTIONS = [
  { label: "İngilis", value: "en-US" },
  { label: "Portuqal", value: "pt" },
  { label: "İspan", value: "es" },
  { label: "Fransız", value: "fr" },
  { label: "Alman", value: "de" },
  { label: "İtalyan", value: "it" },
  { label: "Yapon", value: "ja" },
  { label: "Koreya", value: "ko" },
  { label: "Çin", value: "zh" },
  { label: "Rus", value: "ru" },
  ...
  return (
    <section className="mb-5 flex flex-col items-center text-center sm:mb-6">
      <h1 className="max-w-4xl text-2xl font-semibold tracking-normal text-foreground sm:text-3xl">
        Bu gün hansı təqdimatı yaratmaq istəyirsiniz?
      </h1>
    </section>
  );
}
...
function PresentationFavoriteButton({
  ...
  return (
    <button
      ...
      aria-label={
        file.isFavorited
          ? `${file.name} sevimlilərdən çıxar`
          : `${file.name} sevimlilərə əlavə et`
      }
      aria-pressed={file.isFavorited}
      title={file.isFavorited ? "Sevimlilərdən çıxar" : "Sevimlilərə əlavə et"}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          ...
          >
            ...
            <span className="sr-only">Təqdimat əməliyyatları</span>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-52">
          ...
          >
            ...
            {file.isFavorited ? "Sevimlilərdən çıxar" : "Sevimlilərə əlavə et"}
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
...
function PresentationProjectFilesSection({
  ...
  const tabs: { id: LibraryTab; label: string; icon: LucideIcon }[] = [
    { id: "all", label: "Hamısı", icon: Archive },
    { id: "recent", label: "Son baxılanlar", icon: Clock3 },
    { id: "favorites", label: "Sevimlilər", icon: Star },
  ];
  const sortOptions: { id: SortBy; label: string; icon: LucideIcon }[] = [
    { id: "date-desc", label: "Ən yeni əvvəlcə", icon: Clock3 },
    { id: "date-asc", label: "Ən köhnə əvvəlcə", icon: Clock3 },
    { id: "name-asc", label: "Ad A-Z", icon: Grid2X2 },
    { id: "name-desc", label: "Ad Z-A", icon: Grid2X2 },
  ];
  ...
  const isRenamePending = renameTarget
    ? (files.find((file) => file.id === renameTarget.id)?.isRenamePending ?? renameTarget.isRenamePending)
    : false;
  const isDeletePending = deleteTarget
    ? (files.find((file) => file.id === deleteTarget.id)?.isDeletePending ?? deleteTarget.isDeletePending)
    : false;
    ...
    <>
              ...
              <div className="order-2 shrink-0 sm:order-1">
                ...
                >
                  ...
                  >
                    ...
                    <span className="sr-only">Faylları axtar</span>
                  </Button>
                  <div
                    className={cn(
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      ...
                      <span className="sr-only">Sırala və filtrlə</span>
                  ...
                  <DropdownMenuContent align="end" className="w-52">
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuLabel>Növ</DropdownMenuLabel>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center px-4 py-16 text-center">
              ...
              <p className="mb-2 text-sm font-medium text-foreground">
                Hələ təqdimat yoxdur
              </p>
              <p className="mb-6 text-sm text-muted-foreground">
                Başlamaq üçün ilk təqdimatınızı yaradın
              </p>
            </div>
          ) : viewMode === "grid" ? (
      ...
      >
        <CredenzaContent className="sm:max-w-md">
          <CredenzaHeader>
            <CredenzaTitle>Təqdimatın adını dəyiş</CredenzaTitle>
            <CredenzaDescription>
              Bu təqdimat üçün yeni ad seçin.
            </CredenzaDescription>
          </CredenzaHeader>
          <Input
            value={renameValue}
            onChange={(event) => setRenameValue(event.target.value)}
            placeholder="Yeni təqdimat adı daxil edin"
            autoFocus
          />
          <CredenzaFooter>
            <Button
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Təqdimatı sil</AlertDialogTitle>
            <AlertDialogDescription>
              "{deleteTarget?.name}" tamamilə silinəcək. Bu əməliyyatı geri qaytarmaq mümkün deyil.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export function PresentationDashboard() {
  ...
  const favoriteMutation = useMutation({
    mutationFn: async ({ documentId }: FavoriteMutationVariables) => {
      ...
      if (!result.success) {
        throw new Error(result.message || "Sevimli statusunu yeniləmək alınmadı");
      }
    ...
    onError: (error, _variables, context) => {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Sevimli statusunu yeniləmək alınmadı",
      );
    },
  ...
  const renameMutation = useMutation({
    mutationFn: async ({
      ...
      if (!result.success) {
        throw new Error(result.message || "Təqdimatın adını dəyişmək alınmadı");
      }
    ...
    onError: (error, _variables, context) => {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Təqdimatın adını dəyişmək alınmadı",
      );
    },
    onSuccess: () => {
      toast.success("Təqdimatın adı dəyişdirildi");
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: PRESENTATIONS_QUERY_KEY });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async ({ documentId }: { documentId: string }) => {
      ...
      if (!result.success) {
        throw new Error(result.message || "Təqdimatı silmək alınmadı");
      }
    ...
    onError: (error, _variables, context) => {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Təqdimatı silmək alınmadı",
      );
    },
    onSuccess: () => {
      toast.success("Təqdimat silindi");
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: PRESENTATIONS_QUERY_KEY });
    },
  ...
  const duplicateMutation = useMutation({
    mutationFn: async ({ documentId }: { documentId: string }) => {
      ...
      if (!result.success || !result.presentation) {
        throw new Error(result.message || "Təqdimatı dublikat etmək alınmadı");
      }
    ...
    onSuccess: (presentation) => {
      queryClient.invalidateQueries({ queryKey: PRESENTATIONS_QUERY_KEY });
      toast.success("Təqdimat dublikat edildi");
    ...
    onError: (error) => {
      toast.error(
        error instanceof Error
          ? error.message
          : "Təqdimatı dublikat etmək alınmadı",
      );
    },
  });
  ...
  const filterOptions = useMemo(
    () => [
      { id: ALL_PRESENTATION_DOCUMENT_TYPES, label: "Hamısı" },
      { id: "PRESENTATION", label: "Təqdimatlar" },
    ],
    [],
  );
  ...
  const handleGenerate = async () => {
    ...
    const title = prompt.substring(0, 50) || "Adsız Təqdimat";
    ...
    try {
      ...
      if (!result.success || !result.presentation) {
        ...
        toast.error(result.message || "Təqdimat yaratmaq alınmadı");
        return;
      }
      ...
      toast.error("Təqdimat yaratmaq alınmadı");
    }
  };
  ...
  return (
    <NotebookPageLayout>
      ...
      <NotebookInputBox
        placeholder="Mövzunuzu təsvir edin və ya məzmununuzu bura yapışdırın. Bizim AI onu cəlbedici təqdimata çevirəcək."
      ...
      >
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          <SettingPill icon={PanelsTopLeft} label={slidesLabel}>
            <DropdownMenuLabel>Slaydlar</DropdownMenuLabel>
            <DropdownMenuRadioGroup
